...
function renderUI() {
  ...
  const prevAddChoice = addPropSelect.value;
...
async function handleCategorySwitch(categoryName) {
    ...
    DOMHelper.setSelectValue(addTagSelect, '');
...
async function handleBatchMergeDuplicates() {
    if (!AppState.currentUserUid) return;
    const dbName = dbSelect.value;
...
function handleUpdateSelectedBatch() {
...
function handleAddPropSelect(e) {
  const selectedProp = e.target.value;
  if (selectedProp) {
    DOMHelper.setSelectDisabled(addTagSelect, false);
    const currentSchema = AppState.metadata.categories[AppState.currentCategory] || { tags: {} };
    ...
    if (singleProps.includes(selectedProp)) {
      ...
      addTagSelect.innerHTML = `<option value="">Tag</option>`;
      sortedTagsForProp.forEach(tag => {
        let isSelected = AppState.currentEntryDraft[selectedProp] === tag;
        addTagSelect.innerHTML += `<option value="${tag}" ${isSelected ? 'selected' : ''}>${tag}</option>`;
      });
    } else {
...
async function handleSaveIndividualEntry() {
  if (!AppState.currentUserUid) return;
...
function setupEventListeners() {
  ...
  deleteBtn.addEventListener('click', handleDeleteSelectedEntries);
  ...
  ['movies', 'songs', 'books', 'travel'].forEach(cat => {
      const el = document.getElementById(`nav-${cat}`);
      if(el) el.addEventListener('click', () => handleCategorySwitch(cat));
  });
  ...
  document.getElementById('sidebar-import-toggle').addEventListener('change', handleImportToggle);
  ...
  document.getElementById('bulk-save-btn').addEventListener('click', handleBulkSaveDraft);
  ...
  document.getElementById('update-selected-btn').addEventListener('click', handleUpdateSelectedBatch);

  addPropSelect.addEventListener('change', handleAddPropSelect);
  addTagSelect.addEventListener('change', handleAddTagSelect);

  document.getElementById('save-movie-btn').addEventListener('click', handleSaveIndividualEntry);
  document.getElementById('save-batch-btn').addEventListener('click', handleSaveBulkEntries);
  document.getElementById('compare-delete-btn').addEventListener('click', handleCompareDelete);
  ...
  document.getElementById('view-props-btn').addEventListener('click', handleViewProps);
  ...
  manageSaveBtn.addEventListener('click', handleManageSave);
  manageDeleteBtn.addEventListener('click', handleManageDelete);
  ...
  document.getElementById('duplicate-save-btn').addEventListener('click', handleDuplicateSave);
  ...
  document.getElementById('modal-update-btn').addEventListener('click', handleModalUpdate);
