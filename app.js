...
// ----------------------------------------------------
// CUSTOM DROPDOWN UI INJECTION ENGINE
// ----------------------------------------------------
function initializeCustomDropdowns() {
  ...
  Element.prototype.removeAttribute = function(name) {
      ...
      if (this.tagName === 'SELECT' && name === 'multiple') this.dispatchEvent(new CustomEvent('sync-custom-select'));
  };
  ...
  function applyCustomSelect(select) {
      ...
      function syncUI() {
          if (select.hasAttribute('multiple')) { wrapper.classList.add('is-multiple'); return; } 
          else { wrapper.classList.remove('is-multiple'); }
  ...
  const globalObs = new MutationObserver(mutations => {
      mutations.forEach(m => {
          m.addedNodes.forEach(node => {
              ...
              else if (node.querySelectorAll) node.querySelectorAll('select').forEach(applyCustomSelect);
          });
      });
  });
  ...
  matches.forEach(tMovie => {
    ...
    appMetadata.properties.forEach(p => {
       if(mMovie[p] || tMovie[p]) {
         ...
         rowsHtml += `<tr><td style="font-weight: 500;">${p}</td><td>${mVal}</td><td>${tVal}</td></tr>`;
       }
    });
    if (mMovie.notes || tMovie.notes) rowsHtml += `<tr><td style="font-weight: 500;">Notes</td><td>${mMovie.notes || '-'}</td><td>${tMovie.notes || '-'}</td></tr>`;
...
function renderManageTagsTable() {
  ...
  if (!prop) { manageEditBtn.disabled = true; manageDeleteBtn.disabled = true; return; }
...
async function fetchGitInfo() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      buildEl.innerText = "Unavailable"; commitEl.innerText = "Unavailable";
      dateEl.innerText = "Unavailable"; timeEl.innerText = "Unavailable";
    }
  } catch (error) {
    buildEl.innerText = "Error"; commitEl.innerText = "Error";
    dateEl.innerText = "Error"; timeEl.innerText = "Error";
  }
}

// ----------------------------------------------------
// PILL BOX RENDER ENGINE 
// ----------------------------------------------------
function renderMultiTagPills(prop) {
  const container = document.getElementById('multi-tag-display');
  container.innerHTML = '';
  const values = currentMovieDraft[prop] || [];
  
  if (values.length === 0) {
    container.innerHTML = '<span style="color: var(--muted); font-size: 0.85rem; padding: 5px;">No tags selected</span>';
    return;
  }
  ...
  values.forEach(val => {
    ...
    pill.innerHTML = `<span>${val}</span> <i class="fa-solid fa-xmark remove-pill" style="margin-left:8px; cursor:pointer;" data-val="${val}"></i>`;
  ...
  container.querySelectorAll('.remove-pill').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      currentMovieDraft[prop] = currentMovieDraft[prop].filter(v => v !== valToRemove);
      if(currentMovieDraft[prop].length === 0) delete currentMovieDraft[prop];
      
      const tagSelect = document.getElementById('add-tag-select');
      Array.from(tagSelect.options).forEach(opt => {
        if (opt.value === valToRemove) opt.selected = false;
      });
      tagSelect.dispatchEvent(new CustomEvent('sync-custom-select'));
      renderMultiTagPills(prop);
    });
  });
}

// ----------------------------------------------------
// MODAL LOGIC (READ ONLY TAGS FEATURE)
// ----------------------------------------------------
function openModal(movieId, isEditable) {
  ...
  appMetadata.properties.forEach(prop => {
    ...
    let readOnlyDiv = document.createElement('div');
    readOnlyDiv.className = 'read-only-tags';
    readOnlyDiv.style.flex = '1';
    
    let vals = [];
    if (Array.isArray(movie[prop])) vals = movie[prop];
    else if (movie[prop]) vals = [movie[prop]];
    ...
    if (vals.length > 0) {
      vals.forEach(v => {
        ...
        p.innerText = v;
        readOnlyDiv.appendChild(p);
      });
    } else {
      readOnlyDiv.innerHTML = '<span style="color:var(--muted); padding-top: 8px;">-</span>';
    }
    ...
    if (!singleProps.includes(prop)) { select.setAttribute('multiple', 'multiple'); } 
    else { select.innerHTML = `<option value="">--</option>`; }
    
    (appMetadata.tags[prop] || []).forEach(tag => {
      let isSelected = false;
      if (Array.isArray(movie[prop])) isSelected = movie[prop].includes(tag);
      else isSelected = movie[prop] === tag;
...
function enableEditingMode() {
  document.getElementById('modal-edit-toggle').className = "fa-solid fa-pen icon-btn"; 
  ...
  document.querySelectorAll('.read-only-tags').forEach(el => el.classList.add('hidden'));
  document.querySelectorAll('.edit-mode-container').forEach(el => el.classList.remove('hidden'));
...
function setupEventListeners() {
  ...
  document.getElementById('add-prop-select').addEventListener('change', (e) => {
    ...
    if (selectedProp) {
      ...
      if(!singleProps.includes(selectedProp)) renderMultiTagPills(selectedProp);
    } else { 
      tagSelect.innerHTML = `<option value="">Tag</option>`;
  ...
  document.getElementById('save-movie-btn').addEventListener('click', async () => {
    ...
    if (bulkMoviesDraft.length > 0) {
      ...
      try {
        ...
        document.getElementById('multi-tag-display').classList.add('hidden');
  ...
  document.getElementById('overlap-btn').addEventListener('click', async () => {
    ...
    matches.forEach(tMovie => {
      ...
      appMetadata.properties.forEach(p => {
        ...
        if (mainEmpty && tempHasVal) { updatedData[p] = tMovie[p]; hasUpdates = true; }
      });
  ...
  document.getElementById('modal-update-btn').addEventListener('click', async () => {
    ...
    try {
      ...
      await loadMovies(); // Critical: Load new state before redrawing the modal!
      openModal(activeModalMovieId, false); // Triggers completely fresh redraw into Pill Mode
  ...
  customTagInput.addEventListener('input', (e) => { customAddBtn.disabled = e.target.value.trim().length === 0; });
  ...
  document.getElementById('view-props-btn').addEventListener('click', () => {
    ...
    appMetadata.properties.forEach(prop => { managePropSelect.innerHTML += `<option value="${prop}">${prop}</option>`; });
    manageTagsBody.innerHTML = '';
  ...
  document.getElementById('close-manage-props-modal').addEventListener('click', () => { managePropsModal.classList.add('hidden'); });
  managePropSelect.addEventListener('change', renderManageTagsTable);
  manageEditBtn.addEventListener('click', () => {
    document.querySelectorAll('.manage-tag-input').forEach(input => { input.disabled = false; input.classList.add('editable'); });
  ...
  customAddBtn.addEventListener('click', async () => {
    ...
    if (propChoice === "Property") {
      tagsToAdd.forEach(tagString => {
        if (!appMetadata.properties.includes(tagString)) { appMetadata.properties.push(tagString); appMetadata.tags[tagString] = []; updated = true; }
      });
    } else {
      if (!appMetadata.tags[propChoice]) appMetadata.tags[propChoice] = [];
      tagsToAdd.forEach(tagString => {
        if (!appMetadata.tags[propChoice].includes(tagString)) { appMetadata.tags[propChoice].push(tagString); updated = true; }
      });
    }
    if (updated) {
  ...
  document.getElementById('search-input').addEventListener('keyup', (e) => { if (e.key === 'Enter') triggerActiveFilter(); });
...
function triggerActiveFilter() {
  ...
  if(isCommitsOpen) renderTable(filteredMovies.filter(m => m.isMerged === false), "commits-body", true);
  else if (isDatabaseOpen) renderTable(filteredMovies.filter(m => m.isMerged !== false), "table-body", false);
}
